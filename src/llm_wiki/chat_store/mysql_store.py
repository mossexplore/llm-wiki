...
def create_session(title: str = "新会话") -> dict:
    ...
    with _connection() as conn:
        conn.execute(
            _sql_text("INSERT INTO t_chat_sessions (id, title, created_at, updated_at) VALUES (:id,:title,:created_at,:updated_at)"),
...
def delete_session(session_id: str) -> bool:
    with _connection() as conn:
        ...
        conn.execute(_sql_text("DELETE FROM t_chat_feedback WHERE session_id=:session_id"), {"session_id": session_id})
        return deleted
...
def clear_sessions() -> dict:
    with _connection() as conn:
        ...
        feedback = conn.execute(_sql_text("SELECT count(*) AS n FROM t_chat_feedback")).mappings().one()["n"]
        conn.execute(_sql_text("DELETE FROM t_chat_feedback"))
...
def message_exists(message_id: str) -> dict | None:
    with _connection() as conn:
        row = conn.execute(
            _sql_text("SELECT id, session_id, role FROM t_chat_messages WHERE id=:message_id"),
...
def set_feedback(message_id: str, session_id: str, rating: str, reason: str | None = None) -> dict:
    ...
    with _connection() as conn:
        existing = conn.execute(
            _sql_text("SELECT id FROM t_chat_feedback WHERE message_id=:message_id"),
            {"message_id": message_id},
        ).mappings().first()
        if existing:
            conn.execute(
                _sql_text("""UPDATE t_chat_feedback
            ...
            fid = new_id()
            conn.execute(
                _sql_text("""INSERT INTO t_chat_feedback
