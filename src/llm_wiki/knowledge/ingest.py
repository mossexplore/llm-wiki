...
用法:
    python -m llm_wiki.knowledge.ingest raw_note.txt              # --id 缺省,用时间戳自动命名
    python -m llm_wiki.knowledge.ingest raw_note.txt --id INC-1234  # 也可手动指定(如有工单号)
    cat note.txt | python -m llm_wiki.knowledge.ingest -
...
from llm_wiki.common.llm import client_and_model
