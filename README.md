...
LLM Wiki 是一个面向日志排查、线上故障定位和运维知识沉淀的轻量级 LLM Wiki。它把原始排查记录整理成可追溯的 Markdown 知识库，并通过精确的错误特征串帮助工程师在 Web 页面中找到已复核的解决方案。
...
- 写入、复核、检索、编辑、删除和图谱查看均通过 Web 页面完成。
...
知识写入、复核、列表、检索、编辑、删除和图谱查看都在 Web 页面中完成；不需要手动执行 Python 脚本进行日常操作。
