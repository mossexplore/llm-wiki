...
2. 创建本地配置文件：
   ...
   **存储后端**：`storage.backend` 默认为 `sqlite`，检索索引写到 `index/search.db`，对话数据写到 `db/chat.db`。改为 `mysql` 并填写 `storage.mysql` 后，检索索引和对话运行数据会使用 MySQL；`storage.auto_reindex_on_startup` 默认为 `true`，控制服务启动时是否从 `wiki/cases/` 整库重建检索索引；MySQL 连接由 SQLAlchemy Core 管理连接池和事务，底层驱动为 `mysql+pymysql`。环境变量 `LOG_WIKI_STORAGE_BACKEND`、`LOG_WIKI_AUTO_REINDEX_ON_STARTUP` 与 `LOG_WIKI_MYSQL_*` 可覆盖同名配置。
