...
│   ├── backend/              # FastAPI 后端、API 路由和静态前端挂载
│   ├── knowledge/            # 入库、检索、图谱、Agent 与 OKF 检查
│   ├── search_index/         # 检索索引入口与 SQLite/MySQL 实现
│   ├── chat_store/           # 对话持久化入口与 SQLite/MySQL 实现
│   └── common/               # 存储配置与日志配置等共享模块
├── frontend/                 # 静态前端，无构建步骤
│   ├── index.html            # 单页应用入口
│   ├── css/styles.css        # 页面样式
...
├── pyproject.toml            # Python 包元数据与命令行入口
└── requirements.txt          # 兼容传统 pip -r 安装
```
...
1. 确认 `uvicorn --app-dir src llm_wiki.backend.server:app --port 8000` 能正常启动。
