...
│   ├── chat_store.py         # 对话持久化入口，按配置选择后端
│   ├── search_index.py       # 检索索引入口，按配置选择后端
│   ├── chat_storage/         # 对话存储 SQLite/MySQL 实现
│   ├── knowledge_base/       # 入库、检索、图谱、Agent 与 OKF 检查
│   ├── search_indexing/      # 检索索引 SQLite/MySQL 实现
│   └── shared/               # 存储配置与日志配置等共享模块
├── backend/                  # FastAPI 后端
...
2. 创建本地配置文件：
   ...
   **存储后端**：`storage.backend` 默认为 `sqlite`，检索索引写到 `index/search.db`，对话数据写到 `db/chat.db`。改为 `mysql` 并填写 `storage.mysql` 后，检索索引和对话运行数据会使用 MySQL；MySQL 连接由 SQLAlchemy Core 管理连接池和事务，底层驱动为 `mysql+pymysql`。环境变量 `LOG_WIKI_STORAGE_BACKEND` 与 `LOG_WIKI_MYSQL_*` 可覆盖同名配置。
...
检索索引由 `wiki/cases/*.md` 派生（Markdown 始终是权威源），入库/更新/删除时自动同步、服务启动时整库重建。默认 SQLite 场景下 FTS5 不可用时会自动回退到纯文件扫描，功能不变。索引表结构、查询示例和 MySQL 配置说明见 [db/README.md](db/README.md)。
...
入库管线默认读取仓库根目录的 `config.yaml`，可用环境变量 `INGEST_CONFIG` 指定其它路径。配置分段：`openai` 段供写入知识的 LLM 抽取使用；`chat` 段（可选）供对话 Agent 使用，以 `openai` 为基底按字段覆盖，省略则与写入共用。`chat.thinking` 默认为启用，设为 `false` 可关闭支持该参数的模型思考模式。`storage` 段控制检索索引和对话数据的存储后端，默认 `sqlite`，可改为 `mysql` 并填写连接信息。
