...
对话页参考 NextChat 的布局：左侧管理会话（新建 / 切换 / 删除），右侧是与 Agent 的多轮交互。回答遵循「先检索、后生成」（RAG）：
...
两条路径都走 `config.yaml` 里 OpenAI 兼容接口的真流式；区别只在于「是否把检索到的 wiki 资料注入上下文」。命中知识库时的自定义提示词可用环境变量 `CHAT_WIKI_PROMPT` 覆盖，未命中兜底提示词用 `CHAT_SYSTEM_PROMPT`。
