...
2. 创建本地配置文件：
   ...
   **存储后端**：`storage.backend` 默认为 `sqlite`，检索索引写到 `index/search.db`，对话数据写到 `db/chat.db`。改为 `mysql` 并填写 `storage.mysql` 后，检索索引和对话运行数据会使用 MySQL；环境变量 `LOG_WIKI_STORAGE_BACKEND` 与 `LOG_WIKI_MYSQL_*` 可覆盖同名配置。
