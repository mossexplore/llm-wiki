...
- **检索知识**：粘贴日志、堆栈或错误码后，先按 `signatures` 精确命中，再用 SQLite + FTS5/BM25/trigram 模糊召回；无命中时明确返回暂无案例。
...
![写入知识演示](assets/demo/write-knowledge.gif)

### 知识列表

![知识列表演示](assets/demo/knowledge-list.gif)

### 检索知识

![检索知识演示](assets/demo/search-knowledge.gif)

### 知识图谱

![知识图谱演示](assets/demo/knowledge-graph.gif)

### 对话 Agent

![对话 Agent 演示](assets/demo/chat-agent.gif)
...
│   ├── schema.chat.sql       # 对话会话/消息/反馈运行库 schema
│   ├── schema.sqlite.sql     # SQLite/FTS5 检索索引 schema
│   └── schema.mysql.sql      # MySQL 迁移参考 schema
├── index/                    # 自动生成的 SQLite 检索索引（gitignore，可重建，运行时产生）
├── logs/                     # 本地接口日志与运行日志（gitignore，运行时产生）
├── operations/               # 维护流程说明
│   ├── ingest.md             # 入库流程与护栏说明
│   └── lint.md               # 知识库健康检查说明
├── raw/sources/              # 入库时生成的不可变原始记录（运行时产生）
├── scripts/                  # Web 后端复用的内部模块
│   ├── agent.py              # 对话 Agent：检索决策、RAG 上下文构造、模型流式调用
│   ├── chat_store.py         # 对话运营数据持久化（会话/消息/点赞点踩，SQLite）
│   ├── graph.py              # 从 wiki 文档生成知识图谱数据
│   ├── ingest.py             # 原始记录归档、LLM 抽取、Markdown 生成、索引刷新
│   ├── lint_okf.py           # OKF/wiki 健康检查
│   ├── logging_config.py     # 统一日志配置：logs/app.log 与 logs/access.log
│   ├── query.py              # signature 精确检索 + 模糊召回兜底
│   └── search_index.py       # SQLite + FTS5 检索索引后端
├── backend/                  # FastAPI 后端
│   ├── server.py             # 应用入口：创建 app、挂载路由和静态前端
│   ├── api/                  # 按前端页面拆分的接口模块
│   │   ├── ingest.py         # 写入知识：单条/批量预览与入库
│   │   ├── knowledge.py      # 知识列表：详情、编辑、删除
│   │   ├── search.py         # 检索知识与知识库统计
│   │   ├── graph.py          # 知识图谱
│   │   └── chat.py           # 对话 Agent
│   ├── schemas.py            # API 请求模型
│   └── config.py             # 路径与 scripts/ 加载配置
├── frontend/                 # 静态前端，无构建步骤
│   ├── index.html            # 单页应用入口
│   ├── css/styles.css        # 页面样式
...
- "写入知识"的 LLM 抽取和"对话 Agent"需要 OpenAI 兼容 API key（两者可配不同模型，见配置说明）。
...
2. 创建本地配置文件：
   ...
   **差异化配置**：写入知识用 `openai` 段，对话 Agent 用 `chat` 段。`chat` 以 `openai` 为基底，只需填要改的项（如上面只换 `model`，密钥和网关自动继承 `openai`）；`chat` 段整段省略时，对话与写入共用同一套配置（向后兼容）。`chat.thinking` 控制对话模型是否启用 Think/Thinking 模式，默认 `true`；设为 `false` 时会向兼容网关传 `thinking.type=disabled`，通常可减少首字等待。
...
知识写入、复核、列表、检索、编辑、删除、图谱查看和对话都在 Web 页面中完成；`scripts/` 主要作为后端内部模块使用。
...
检索索引由 `wiki/cases/*.md` 派生（Markdown 始终是权威源），入库/更新/删除时自动同步、服务启动时整库重建。FTS5 不可用时自动回退到纯文件扫描，功能不变。索引表结构、查询示例与迁移（D1 / MySQL）说明见 [db/README.md](db/README.md)。
...
所有会话、用户提问、Agent 回复、时延指标、点赞 / 点踩（含原因）都落库到 `db/chat.db`（运行库，已 gitignore），表结构见 [db/schema.chat.sql](db/schema.chat.sql)，便于后续做对话质量分析、知识盲区发现、性能分析和答案来源统计。
...
入库管线默认读取仓库根目录的 `config.yaml`，可用环境变量 `INGEST_CONFIG` 指定其它路径。配置分段：`openai` 段供写入知识的 LLM 抽取使用；`chat` 段（可选）供对话 Agent 使用，以 `openai` 为基底按字段覆盖，省略则与写入共用。`chat.thinking` 默认为启用，设为 `false` 可关闭支持该参数的模型思考模式。
...
1. 确认 `uvicorn backend.server:app --port 8000` 能正常启动。
